	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		s.setAttribute("rid",rid);
			if(b == true)
			{
				if( rid==1)
				{
					response.sendRedirect("IT/FinanceIndex.jsp");
				}
				else if(rid==2)
				{
					
					response.sendRedirect("IT/HRIndex.jsp");
				}
				else if(rid==3)
				{
					response.sendRedirect("IT/PMIndex.jsp");
				}
				else if(rid==4)
				{
					response.sendRedirect("IT/UserIndex.jsp");
				}
				else if(rid==5)
				{
					response.sendRedirect("IT/DevloperIndex.jsp");
				}
				
			}
			else
			{
				response.sendRedirect("IT/Login.jsp?msg=1");
			}
